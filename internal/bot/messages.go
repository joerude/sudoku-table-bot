...
const helpText = `🧩 <b>Sudoku League</b> — бот ведёт учёт ваших игр в судоку (usdoku.com): очки, места, сезоны, дуэли.

<b>Игроки</b>
/join [имя] — встать в игру (без имени возьму твоё имя в Telegram)
/setnick &lt;ник&gt; — привязать свой ник с usdoku, чтобы результат подтягивался сам
/players — кто играет + их usdoku-ники
/removeplayer &lt;имя&gt; — убрать игрока, прошлые игры сохранятся (админ)

<b>Игра</b>
/newgame &lt;easy|medium|hard|extreme&gt; [hardcore|original] — создать комнату usdoku + ссылка; результат подтянется сам
/result — записать результат вручную (жми игроков по порядку финиша)
/duel — вызвать одного на дуэль 1-на-1 (кнопки игроков → Принять/Отказ); дуэли идут отдельно от сезона
/invite — позвать всех: пинг + ссылка + кнопка «Я в деле»

<b>Статистика</b>
/status — таблица сезона: места, очки, победы, прогресс лидера
/season — текущий сезон: цель и таблица очков по местам
/me — моё: место, очки, победы, среднее время, лучшее, рекорд дуэлей
/history — последние сыгранные игры сезона
/speed [сложность] — рейтинг по среднему времени решения (по умолчанию medium)
/duels — рейтинг дуэлей (победы-поражения, %) + последние дуэли
/export — выгрузить все игры сезона в CSV

<b>Настройки</b> (только админ)
/settings — порог сезона, таблица очков по местам, мин. игроков, напоминания
/backup — скачать базу файлом; чтобы восстановить — пришли файл с подписью /restore`
