...
type HistoryGame struct {
	ID           int64
	SeasonNumber int      // season "number" (not row id)
...
func (s *Store) RecentGames(chatID int64, n int) ([]HistoryGame, error) {
	rows, err := s.db.Query(`
		SELECT g.id, s.number, g.completed_at, COALESCE(g.difficulty,''), COALESCE(g.usdoku_code,'')
	...
	for rows.Next() {
		var g HistoryGame
		if err := rows.Scan(&g.ID, &g.SeasonNumber, &g.CompletedAt, &g.Difficulty, &g.UsdokuCode); err != nil {
			return nil, err
		}
